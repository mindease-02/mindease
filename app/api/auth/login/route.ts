...
export async function POST(req: Request) {
  const body = (await req.json().catch(() => ({}))) as { identifier?: string; name?: string; timeZone?: string; region?: string; language?: string; proactive?: boolean };
  ...
  const state = await loadOrCreate(userId, name, body.timeZone, body.region, isLanguage(body.language) ? body.language : undefined);
