...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { mood?: string; note?: string; skip?: boolean };
  ...
  if (body.skip) {
    ...
    state.arrival = { mood: m.id, label: m.label, hint: m.hint, note: (body.note ?? "").trim().slice(0, 200) || undefined, at: Date.now() };
  }
