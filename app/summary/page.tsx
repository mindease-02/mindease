...
export default async function SummaryPage() {
  ...
  const fmt = (x: number) => new Date(x).toLocaleDateString(lang === "en" ? "en-IN" : `${lang}-IN`, { day: "numeric", month: "short", year: "numeric" });
  ...
  return (
    <div className={`world summary ${display.variable} ${heading.variable} ${body.variable}`}>
      ...
      <main className="container sheet" id="main">
        <div className="sticker alt">{t("sumSticker", lang)}</div>
        <h1 className="display" style={{ fontSize: "clamp(1.4rem, 3vw, 2.2rem)", margin: "14px 0 6px" }}>{t("sumTitle", lang, { name: session.name })}</h1>
        <p className="muted">{t("sumPrep", lang, { date: fmt(now), days: String(days) })}</p>

        <h2 className="display sheet-h">{t("sumScores", lang)}</h2>
        {done.length ? (
          <table className="sheet-table">
            <thead><tr><th>{t("instrument", lang)}</th><th>{t("date", lang)}</th><th>{t("score", lang)}</th><th>{t("range", lang)}</th></tr></thead>
        ...
        ) : <p className="muted">{t("sumNoScreen", lang)}</p>}

        <h2 className="display sheet-h">{t("sumPatterns", lang)}</h2>
        {patterns.length ? (
        ...
        ) : <p className="muted">{t("sumNoPatterns", lang)}</p>}
