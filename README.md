...
| Auto-behaviour | `lib/lifestyle/autoTune.ts` | Nobody configures Ori. Quiet hours come from the inferred sleep window, cadence from the active window and return rhythm, the check-in budget from reliance, signal use from baseline reliability. Explained in plain words in the Mirror. Flow: chat → sign in → mood → chat. |
