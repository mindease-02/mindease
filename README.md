...
Samantha's warmth, 2049's atmosphere, and a design that refuses to be Joi: MindEase is software, says so, measures how much you rely on it, and pulls back when that climbs.
...
| Auto-behaviour | `lib/lifestyle/autoTune.ts` | Nobody configures MindEase. Quiet hours come from the inferred sleep window, cadence from the active window and return rhythm, the check-in budget from reliance, signal use from baseline reliability. Explained in plain words in the Mirror. Flow: chat → sign in → mood → chat. |
