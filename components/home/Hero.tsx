 ...
 * rises word by word in the lower third, film-title style. It stays pinned
 * while the wheel turns into the two ribbons of the next chapter.
 */
export default function Hero({ chatHref, lang }: { chatHref: string; lang: string }) {
  return (
    <Chapter id="top" length={1.8} className="ch-hero" label={t("home", lang)}>
      <div data-beat data-in="0" data-out="1" className="beat beat-title">
        <Reveal className="container hero-inner">
          ...
          <p className="hero-note" data-reveal style={{ ["--d" as string]: "700ms" }}>{sentences(t("footNot", lang), t("priceLine", lang))}</p>
        </Reveal>
      </div>
