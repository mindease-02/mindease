...
export default function MoodOrb({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    ...
    const ball = new THREE.Mesh(new THREE.SphereGeometry(1.15, 96, 96), mat); scene.add(ball);
    const halo = new THREE.Mesh(new THREE.SphereGeometry(1.42, 64, 64), new THREE.MeshBasicMaterial({ color: target.current.accent2, transparent: true, opacity: 0.12, blending: THREE.AdditiveBlending, side: THREE.BackSide, depthWrite: false })); scene.add(halo);
    ...
    const onMove = (e: PointerEvent) => { if (e.pointerType === "touch") return; const r = el.getBoundingClientRect(); px = ((e.clientX - r.left) / r.width - 0.5) * 0.9; py = ((e.clientY - r.top) / r.height - 0.5) * 0.6; };
