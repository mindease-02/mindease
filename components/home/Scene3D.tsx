 ...
 *
 * On load the particles converge from a scattered cloud into the wheel; each
 * chapter can nudge and dim the field through the "me:chapter" event.
 */
...
function makeFrames(n: number, read: number[], ringRx: number, ringRy: number, spread = 1): Record<string, Frame> {
  const r = rand(7);
...
const VERT = `
attribute vec3 posB; attribute vec3 colA; attribute vec3 colB; attribute float sizeA; attribute float sizeB; attribute vec3 scatter;
uniform float uMix; uniform float uTime; uniform float uPixel; uniform float uDim; uniform float uIntro;
...
void main() {
  ...
  // The opening: everything drifts in from a wide scattered cloud.
  p = mix(p, scatter, uIntro * uIntro);
...
/** Camera distance per chapter: a slow dolly between them. */
const ZOOM: Record<string, number> = { hero: 8.2, why: 9.2, demo: 8.4, start: 9.0, details: 10.2 };
const ZOOM_PHONE: Record<string, number> = { hero: 13, why: 10.5, demo: 13.5, start: 9.5, details: 11 };
/** Share of each chapter's scroll during which the field holds its shape before morphing into the next. */
const HOLD: Record<string, number> = { hero: 0.45, why: 0.68, demo: 0.62, start: 0.55, details: 0 };
...
export default function Scene3D() {
  ...
  useEffect(() => {
    ...
    const ringRx = phone ? 2.3 : 4.9, ringRy = phone ? 3.4 : 2.7, spread = phone ? 0.56 : 1;
    let frames = makeFrames(n, read, ringRx, ringRy, spread);
    ...
    // What the chapter on stage asks for: a nudge of the field and a dim, eased in tick.
    let nx = 0, ny = 0, tnx = 0, tny = 0, dim = 1, tdim = 1, chapter = "hero";
    const zooms = phone ? ZOOM_PHONE : ZOOM;
    let zoom = zooms.hero, tzoom = zoom;
    const onScroll = () => {
      const y = window.scrollY + window.innerHeight * 0.5;
      let i = 0; while (i < ORDER.length - 2 && y >= anchors[i + 1]) i++;
      const span = Math.max(1, anchors[i + 1] - anchors[i]);
      const raw = Math.min(1, Math.max(0, (y - anchors[i]) / span));
      // Each chapter holds its shape for most of its scroll, then morphs into the next.
      const hold = HOLD[ORDER[i]] ?? 0;
      const t = Math.min(1, Math.max(0, (raw - hold) / Math.max(0.001, 1 - hold)));
      ...
      tzoom = zooms[ORDER[i]] + (zooms[ORDER[i + 1]] - zooms[ORDER[i]]) * t + past * 2.5;
    ...
    const onChapter = (e: Event) => {
      const d = (e as CustomEvent<{ id: string; fx: number; fy: number; dim: number }>).detail; if (!d) return;
      chapter = d.id; tnx = d.fx || 0; tny = d.fy || 0; tdim = d.dim ?? 1;
    };
    // The ribbon ends and the week marks, projected to the screen so the chart's labels ride on the particles.
    const v = new THREE.Vector3();
    const mark = (name: string, x: number, y: number) => {
      v.set(x, y, 0).applyMatrix4(points.matrixWorld).project(camera);
      document.documentElement.style.setProperty(`--why-${name}-x`, `${((v.x + 1) / 2 * el.clientWidth).toFixed(0)}px`);
      document.documentElement.style.setProperty(`--why-${name}-y`, `${((1 - v.y) / 2 * el.clientHeight).toFixed(0)}px`);
    };
    ...
    const onRead = (e: Event) => {
      ...
      frames = { ...frames, demo: makeFrames(n, read, ringRx, ringRy, spread).demo };
    ...
    let raf = 0, last = performance.now(), visible = true, pending = false, t0 = -1;
    const tick = (now: number) => {
      pending = false;
      const dt = Math.max(0, Math.min(0.05, (now - last) / 1000)); last = now;
      if (t0 < 0) t0 = now;
      if (!reduced) govern(dt);
      if (still) { mix = targetMix; rotX = tRotX; rotY = tRotY; ox = tox; oy = toy; nx = tnx; ny = tny; zoom = tzoom; dim = tdim; }
      ...
      nx += (tnx - nx) * Math.min(1, dt * 3); ny += (tny - ny) * Math.min(1, dt * 3);
      dim += (tdim - dim) * Math.min(1, dt * 3);
      zoom += (tzoom - zoom) * Math.min(1, dt * 3); camera.position.z = zoom;
      points.position.set(ox + nx, oy + ny, 0);
      points.updateMatrixWorld();
      if (chapter === "why") { mark("ppl", 3.3 * spread, 1.3); mark("app", 3.3 * spread, -1.1); mark("w1", -3.2 * spread, -2.05); mark("w8", 3.2 * spread, -2.05); }
      // The opening converges on the clock, not the frame count, so a slow device still settles in under two seconds.
      if (!reduced && now < t0 + 1800) mat.uniforms.uIntro.value = 0.9 * (1 - (now - t0) / 1800); else mat.uniforms.uIntro.value = 0;
      mat.uniforms.uDim.value = (phone ? 0.62 : 1) * dim;
    ...
    window.addEventListener("me:chapter", onChapter);
    ...
    return () => {
      ...
      window.removeEventListener("scroll", onScroll); window.removeEventListener("resize", onResize); window.removeEventListener("pointermove", onPointer); window.removeEventListener("me:read", onRead); window.removeEventListener("me:chapter", onChapter); document.removeEventListener("visibilitychange", onVis);
